...
public class Arbre {
   ...
   public static double demilargeur = 1; // pas de déplacement
   ...
   // Création d'une répartition aléatoire de planètes
   static void insereCorpsAleatoire(Arbre a, int n1, int n2, int n3, double m1, double m2, double m3, int maxcoordonnees) {
      // on crée n1 planetes de masse m1 à vitesse 0 qu'on met dans une liste
      ...
      // on crée n2 planetes de masse m2 à vitesse 0 qu'on met dans la meme
      // liste à la suite
      for (int i = 1; i < n2
         + 1; i++) {
      ...
      for (Planete p : listePlanete) {
         ...
         // j'obtiens un nombre aléatoire entre 0 et 1
         double xAleamaxcoordonnees =
            xAlea
               * maxcoordonnees;
         // j'obtiens un nombre décimal aléatoire entre 0 et maxcoordonnees
         double x = Math.round(xAleamaxcoordonnees);
         // j'obtiens un nombre entier aléatoire entre 0 et maxcoordonnees
         // idem pour y, on relance Math.random() pour retirer un autre nb
         // aleatoire
         double yAlea = Math.random();
         double yAleaMaxCoordonnees =
            yAlea
               * maxcoordonnees;
   ...
   static Arbre insereCorps(double x, double y, Planete planete, Arbre a) {
      ...
      if (a.x == x
         && a.y == y) { // la position correspond à celle voulue
         System.out.println(
            "Bingo x=x et y=y pour "
               + x
               + " "
               + y);
         a.estFeuille = true;
         ...
         if (x > a.x) {
            ...
            if (y > a.y) {
               if (a.se == null) {
                  Arbre b =
                     new Arbre(a.x
                        + (profmax
                           * 2
                           * demilargeur)
                           / (Math.pow(2, a.profondeur)), a.y
                              + (profmax
                                 * 2
                                 * demilargeur)
                                 / (Math.pow(2, a.profondeur)), null, null, null, null, 0, a.profondeur
                                    + 1);
                  a.se = b;
                  ...
                  // Insertion noued fictif qui sera remplacé par la suite
   ...
   // Calcul des vitesses des planètes de l'arbre
   public void calculToutesVitesses(Arbre a) {
      if (a.estFeuille) {
         ...
         // Lecture de bas gauche à bas droite en rotation de l'arbre (parcours
         // préfixe)
         if (a.so != null)
   ...
   public void nouvellePosition(Arbre a) {
      if (this.estFeuille
         && this.bplanete) {// si la norme de la vitesse est trop
                                                // faible
         if (this.planete.vitesse.norme() < Math.pow(10, -7)) {
            insereCorps(this.x, this.y, this.planete, a);
         }
         else {
            // Ma Différenciation des cas de la vitesse suivant la division
            // de la grille
            if (this.planete.vitesse.x > 0) {
               if (this.planete.vitesse.y > Math.sqrt(2)
                  * this.planete.vitesse.x) {
                  insereCorps(
                     this.x,
                     this.y
                        + 2
                  ...
                  / Math.sqrt(2))
                  * this.planete.vitesse.x < this.planete.vitesse.y
                  && this.planete.vitesse.y < Math.sqrt(2)
                     * this.planete.vitesse.x) {
                  insereCorps(
                     this.x
                        + 2
               ...
               else if (-this.planete.vitesse.y > (1
                  / Math.sqrt(2))
                  * this.planete.vitesse.x
                  && -this.planete.vitesse.y < Math.sqrt(2)
                     * this.planete.vitesse.x) {
               ...
               else if (this.planete.vitesse.y < -Math.sqrt(2)
                  * this.planete.vitesse.x) {
                  insereCorps(
                     this.x,
                     this.y
      ...
      else { // on applique ensuite la fonction récursivement à tous les
