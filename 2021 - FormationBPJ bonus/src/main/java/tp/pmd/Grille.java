...
public class Grille extends JPanel {
   ...
   // Permet d'afficher les plan�tes de l'arbre comme des cercles suivant
   // leurs masses par un parcours de celui-ci
   public void parcoursEtAfficheArbre (Graphics g, Arbre b) {
      if (b != null) {
         if (b.estFeuille && b.bplanete == true) {
            g.setColor(b.planete.couleur());
            g.fillOval((int) b.x - diametre / 2, (int) b.y - diametre / 2, diametre, diametre);
         }
         else {
            // Lecture de bas gauche � bas droite en rotation (parcours
            // pr�fixe)
